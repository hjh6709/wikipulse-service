...
## 6. reddit-comments Kafka 토픽 스키마

**확인 대상:** 김찬영 (데이터팀)

**배경:**
댓글 피드 구현을 위해 `reddit-comments` 토픽을 서비스팀도 consume해야 합니다.
현재 mock 스키마로 개발 중이며 실제 필드 구성 확인이 필요합니다.

**결정 필요 사항:**
- `reddit-comments` 토픽 필드 목록 확정 (comment_id, post_id, issue_id, author, body, score, created_at 맞는지)
- `score` 필드명 유지 여부 (Reddit API 원본 필드명과 동일한지)
- 댓글 원문에 sentiment 필드 포함 여부, 또는 sentiment-results와 comment_id로만 연결할지

**영향 범위:** Reddit 댓글 피드 컴포넌트, `mock/comments.json`, Kafka consumer 코드

---

## 7. Kong Gateway 비로그인 경로 예외 설정

**확인 대상:** 윤승호 (DevSecOps)

**배경:**
WikiPulse가 외부 공개 서비스로 랜딩 페이지(`/`)에서 트렌딩 이슈 미리보기를 비로그인으로 제공합니다.
현재 Kong이 모든 요청에 JWT 검증을 하는데, 일부 경로는 인증 없이 통과시켜야 합니다.

**결정 필요 사항:**
- `GET /issues?preview=true` 를 Kong JWT 검증 예외로 설정
- 예외 처리 방식 (Kong 플러그인 설정 or 별도 public 엔드포인트 분리)

**영향 범위:** Kong Gateway 라우팅 설정, `GET /issues` 엔드포인트

---

## 8. CDN 선택 — CloudFront vs Cloudflare
...
**배경:**
랜딩 페이지 hero 섹션의 한 줄 태그라인 후보 3가지:
1. "실시간 Wikipedia 이슈 트래커"
2. "Wikipedia가 들썩이는 순간을 포착합니다"
3. "지금 Wikipedia에서 무슨 일이 벌어지고 있나요?" ← 현재 적용 중
