...
```
배경: stone-950 (가장 어두움) → stone-900 → stone-800
텍스트: stone-100 (밝음) → stone-400 (흐릿함) → stone-600 (더 흐릿함)
액센트(강조): amber-500 (노란빛 주황) — 버튼, 활성 필터, 포커스 링
상태 색상:
  발생: sky-400/sky-500 (하늘색)
  확산: yellow-400/yellow-500
  정점: red-400/red-500
  소강: stone-400/stone-500
```

**투명도 접미사**: `bg-sky-500/15` → sky-500 색상의 15% 불투명도 배경

**좌측 보더**: `border-l-4 border-l-sky-500` → 상태별 4px 왼쪽 테두리
...
<ResponsiveContainer width="100%" height={200}>
  <LineChart data={spikePoints}>   {/* [{ time: "14:00", edits: 20 }, ...] */}
    <XAxis dataKey="time" />
    <YAxis />
    <Line type="monotone" dataKey="edits" stroke="#d97706" />  {/* amber 색상 */}
  </LineChart>
</ResponsiveContainer>
```
